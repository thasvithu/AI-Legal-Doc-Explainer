...
.brand-badge {width:34px;height:34px;display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,#6a5acd,#8f7bff);color:#fff;font-weight:700;border-radius:10px;font-size:.9rem;letter-spacing:.5px;box-shadow:0 4px 10px -2px rgba(0,0,0,.5);}
.status-grid {display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:6px;margin-top:.25rem;}
.status-pill {background:#1c232e;border:1px solid #2d3441;padding:6px 8px;border-radius:8px;font-size:.58rem;line-height:1.1;letter-spacing:.4px;text-transform:uppercase;display:flex;flex-direction:column;gap:2px;}
.status-pill span.value {font-size:.72rem;font-weight:600;color:#e2e6ee;letter-spacing:0;}
.progress-wrap {margin-top:.5rem;}
.progress-bar {height:6px;border-radius:4px;background:#2a323d;overflow:hidden;}
.progress-bar > div {height:100%;background:linear-gradient(90deg,#6a5acd,#ffb347);width:0%;transition:width .6s ease;}
.nav-links {display:flex;flex-direction:column;gap:4px;margin-top:8px;}
.nav-links a {text-decoration:none;font-size:.7rem;padding:6px 10px;border:1px solid #252d38;border-radius:8px;color:#c9d2dd;background:#141a22;display:flex;justify-content:space-between;align-items:center;transition:background .15s,border .15s;}
.nav-links a:hover {background:#1d2530;border-color:#2e3642;}
.nav-links a span.badge {background:#252f3a;color:#aeb8c4;padding:2px 6px;border-radius:12px;font-size:.55rem;}
.glassy {background:rgba(255,255,255,0.04);border:1px solid #2a3140;padding:.75rem .8rem;border-radius:14px;box-shadow:0 4px 14px -6px rgba(0,0,0,.55);}
...
def sidebar(config: AppConfig):
    ...
    st.sidebar.markdown(
        f"<div class='progress-wrap'><div style='display:flex;justify-content:space-between;font-size:.55rem;opacity:.7;margin-bottom:3px;'><span>Pipeline Progress</span><span>{progress_pct}%</span></div><div class='progress-bar'><div style='width:{progress_pct}%;'></div></div></div>",
        unsafe_allow_html=True,
    )
    st.sidebar.markdown(
        "<div class='nav-links'>"
